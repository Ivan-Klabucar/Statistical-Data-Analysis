...
par(mar=c(12,5,1,1))
boxplot(Data_Value ~ Short_Question_Text, onlyHealthOutcomes, col = myGreen, las=2, xlab="")
...
par(mar=c(12,5,1,1))
boxplot(Total.percentage ~ Short_Question_Text, data = onlyHealthOutcomes, col = myGreen, las=2, xlab="")
mtext("Short_Question_Text", side=1, line=10.5)
```
...
barplot(counts, names = c("Cholesterol scr", "Health insurance", "BP medication", "Annual checkup"), col = c(myBlue, myGreen), beside = TRUE)
...
barplot(c(Illinois_COPD$Total.percentage, Iowa_COPD$Total.percentage, S_Dakota_COPD$Total.percentage), col = c("indianred3", myBlue, myGreen), names = c("Illinois", "Iowa", "South Dakota"))
```
